...
use windows::{
  ...
  Win32::{
    Foundation::*,
    Globalization::*,
    Graphics::{Gdi::*, DirectComposition::*},
...
impl InnerWebView {
  ...
  fn new_in_hwnd(
    ...
    let cr = Self::create_controller(hwnd, &env, attributes.incognito, background_color, composition_mode)?;
  ...
  fn create_composition_controller(
    ...
    let env3: ICoreWebView2Environment3 = env.cast().map_err(|e| {
      crate::Error::WebView2Error(webview2_com::Error::WindowsError(e))
    })?;
    ...
    // Store raw COM pointer in global static for host app retrieval
    {
      use windows::core::Interface;
      let ptr = comp_controller.as_raw() as isize;
      crate::LAST_COMP_CONTROLLER_PTR.store(ptr, std::sync::atomic::Ordering::SeqCst);
    }

    // The composition controller implements ICoreWebView2Controller
    let controller: ICoreWebView2Controller = comp_controller.cast().map_err(|e| {
      crate::Error::WebView2Error(webview2_com::Error::WindowsError(e))
    })?;
  ...
  unsafe fn setup_dcomp(
    ...
    let device: IDCompositionDevice = DCompositionCreateDevice2(None).map_err(|e| {
      crate::Error::WebView2Error(webview2_com::Error::WindowsError(e))
    })?;
    ...
    let visual_unknown: windows::core::IUnknown = visual.cast().map_err(|e| {
      crate::Error::WebView2Error(webview2_com::Error::WindowsError(e))
    })?;
    comp_controller.SetRootVisualTarget(&visual_unknown).map_err(|e| {
      crate::Error::WebView2Error(webview2_com::Error::WindowsError(e))
    })?;
    ...
    Ok(DCompState { device, target, visual })
  }
