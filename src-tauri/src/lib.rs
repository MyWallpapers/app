...
fn mw_init_script() -> String {
    format!(
        ...
        std::env::consts::OS, std::env::consts::ARCH, env!("CARGO_PKG_VERSION"), tauri::VERSION, cfg!(debug_assertions),
    )
}
...
pub fn main() {
    ...
    info!("[main] Starting MyWallpaper Desktop v{} ({}/{})", env!("CARGO_PKG_VERSION"), std::env::consts::OS, std::env::consts::ARCH);
    start_with_tauri_webview();
}

fn start_with_tauri_webview() {
    use tauri::{Emitter, Listener, Manager, webview::PageLoadEvent};
    ...
    let app = tauri::Builder::default()
        ...
        .plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
            if let Some(window) = app.get_webview_window("main") {
                args.into_iter().filter(|a| a.starts_with("mywallpaper://")).for_each(|url| {
            ...
            app.listen("deep-link://new-url", move |event| {
                if let Ok(urls) = serde_json::from_str::<Vec<String>>(event.payload()) {
                    if let Some(window) = deep_link_handle.get_webview_window("main") {
                        urls.into_iter().filter(|u| u.starts_with("mywallpaper://")).for_each(|url| {
                            let _ = window.emit("deep-link", url);
                        });
                    }
                }
